...
class SingleOutUGen( val name: String, val rate: Rate, val inputs: IIdxSeq[ UGenIn ]) extends UGen with UGenProxy {
...
//case class GEOutProxy[ R <: Rate ]( source: AnyGE, outputIndex: Int )
//extends /* UGenIn[ R ] with UGenProxy[ S ] */ GE[ R, UGenOutProxy ] with UGenIn {
//   override def toString = source.toString + ".\\(" + outputIndex + ")"
////   def displayName = source.displayName + " \\ " + outputIndex
//
//   def expand: IIdxSeq[ UGenOutProxy ]
//}
