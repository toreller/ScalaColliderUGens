...
final case class RichFloat private[synth]( f: Float ) extends NAryDoubleOps {
   import RichFloat._
   import RichDouble._
...
//      rd_linlin( f, srcLo, srcHi, dstLo, dstHi )
...
//      rd_linexp( f, srcLo, srcHi, dstLo, dstHi )
...
/**
 * TODO: min( b: GE ), max( b: GE ), and round( b: GE ) are currenlty
 * causing conflicts with scala.RichXYZ, the most retarded idea ever
 * put into practise... once we have some energy, we should file
 * a SIP because scala.RichXYZ is a real PITA
 */
...
final case class RichDouble private[synth]( protected val d: Double ) extends NAryDoubleOps {
   import RichDouble.LOG2
