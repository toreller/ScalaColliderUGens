...
object Line {
//   def ar: Line with GE[ LineUGen[ audio ]] = ar( )
//   def kr: Line with GE[ LineUGen[ control ]] = kr( )
   def ar(start: GE[AnyUGenIn] = 0.0f, end: GE[AnyUGenIn] = 1.0f, dur: GE[AnyUGenIn] = 1.0f, doneAction: GE[AnyUGenIn] = doNothing) :
      Line with GE[ LineUGen with AudioRated ] = {
//      apply[audio](audio, start, end, dur, doneAction)
      error( "NOT YET" )
   }
   def kr(start: GE[AnyUGenIn] = 0.0f, end: GE[AnyUGenIn] = 1.0f, dur: GE[AnyUGenIn] = 1.0f, doneAction: GE[AnyUGenIn] = doNothing) :
      Line with GE[ LineUGen with ControlRated ] = {
//      apply[control](control, start, end, dur, doneAction)
      error( "NOT YET" )
   }
}

trait Line {
   def start: GE[AnyUGenIn]
   def end: GE[AnyUGenIn]
   def dur: GE[AnyUGenIn]
   def doneAction: GE[AnyUGenIn]
}
...
trait LineUGen {
   ...
   def end: AnyUGenIn
   def dur: AnyUGenIn
   def doneAction: AnyUGenIn
}
//
//extends SingleOutUGen[audio](IIdxSeq(start, end, dur, doneAction)) with HasSideEffect with HasDoneFlag