...
import de.sciss.synth.{ AnyUGenIn, scalar, control, audio, Constant => c, GE, Rate, RichFloat, HasSideEffect, SingleOutUGen, SynthGraph,
                        UGenIn }
 ...
 */
object MulAdd {
   def ar( in: GE[ UGenIn[ audio ]],   mul: GE[ AnyUGenIn ], add: GE[ AnyUGenIn ]) : MulAdd[ audio ]   = apply[ audio ](   audio,   in, mul, add )
   def kr( in: GE[ UGenIn[ control ]], mul: GE[ AnyUGenIn ], add: GE[ AnyUGenIn ]) : MulAdd[ control ] = apply[ control ]( control, in, mul, add )
   def ir( in: GE[ UGenIn[ scalar ]],  mul: GE[ AnyUGenIn ], add: GE[ AnyUGenIn ]) : MulAdd[ scalar ]  = apply[ scalar ](  scalar,  in, mul, add )
}

case class MulAdd[ R <: Rate ]( rate: R, in: GE[ UGenIn[ R ]], mul: GE[ AnyUGenIn ], add: GE[ AnyUGenIn ]) extends GE[ MulAddUGen[ R ]] {
   def expand = {
      val _in     = in.expand
      val _mul    = mul.expand
      val _add    = add.expand
      ...
      val _exp_   = max( _sz_in, _sz_mul, _sz_add )
