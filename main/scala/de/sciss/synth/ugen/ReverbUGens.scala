...
case class FreeVerb2(left: AnyGE, right: AnyGE, mix: AnyGE, room: AnyGE, damp: AnyGE) extends MultiOutUGenSource[audio, FreeVerb2UGen] with AudioRated {
   protected def expandUGens = {
...
case class FreeVerb2UGen(left: AnyUGenIn, right: AnyUGenIn, mix: AnyUGenIn, room: AnyUGenIn, damp: AnyUGenIn) extends MultiOutUGen[audio](IIdxSeq.fill(2)(audio), IIdxSeq(left, right, mix, room, damp)) with AudioRated
object GVerb {
...
case class GVerb(in: AnyGE, roomSize: AnyGE, revTime: AnyGE, damping: AnyGE, inputBW: AnyGE, spread: AnyGE, dryLevel: AnyGE, earlyRefLevel: AnyGE, tailLevel: AnyGE, maxRoomSize: AnyGE) extends MultiOutUGenSource[audio, GVerbUGen] with AudioRated {
   protected def expandUGens = {
      val _in: IIdxSeq[AnyUGenIn] = in.expand
