...
case class DiskIn(numChannels: Int, buf: GE[AnyUGenIn], loop: GE[AnyUGenIn]) extends Expands[DiskInUGen] with AudioRated {
   def expand = {
      val _buf = buf.expand
...
object DiskOut {
   def ar(buf: GE[AnyUGenIn], in: Expands[GE[UGenIn[audio]]]) = apply(buf, in)
}
case class DiskOut(buf: GE[AnyUGenIn], in: Expands[GE[UGenIn[audio]]]) extends GE[DiskOutUGen] with AudioRated {
   def expand = {
      val _buf = buf.expand
      val _in = in.expand
      val _sz_buf = _buf.size
...
case class VDiskIn(numChannels: Int, buf: GE[AnyUGenIn], speed: GE[AnyUGenIn], loop: GE[AnyUGenIn], sendID: GE[AnyUGenIn]) extends Expands[VDiskInUGen] {
   def expand = {
      val _buf = buf.expand
