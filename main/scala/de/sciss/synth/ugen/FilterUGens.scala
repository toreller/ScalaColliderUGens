 ...
 * @see [[de.sciss.synth.ugen.Ramp]]
 */
case class Lag[ R <: Rate ](rate: R, in: AnyGE, time: AnyGE) extends SingleOutUGenSource[R, LagUGen] {
   protected def expandUGens = {
      val _in: IIdxSeq[UGenIn] = in.expand
...
case class LPF(rate: Rate, in: AnyGE, freq: AnyGE) extends SingleOutUGenSource[LPFUGen] {
   protected def expandUGens = {
      val _in: IIdxSeq[UGenIn] = in.expand
      val _freq: IIdxSeq[UGenIn] = freq.expand
      val _sz_in = _in.size
