...
object DetectSilence {
   /**
    * @param in              input signal to be measured.
    * @param amp             minimum amplitude threshold which must be exceeded for the input signal
    *                        to be considered non-silent.
    * @param dur             The duration in seconds for which the input signal must be continuously smaller than or equal
    *                        to the threshold to be considered silent.
    * @param doneAction      an action to be performed when the output changes from zero to one (silence detected).
    */
