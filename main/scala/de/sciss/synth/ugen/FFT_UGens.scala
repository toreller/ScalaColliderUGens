...
case class IFFT[R <: Rate](rate: R, chain: AnyGE, winType: AnyGE, winSize: AnyGE) extends SingleOutUGenSource[R, IFFTUGen[R]] {
   protected def expandUGens = {
...
case class FFTTrigger(buf: AnyGE, hop: AnyGE = 0.5f, polar: AnyGE = 0.0f) extends SingleOutUGenSource[control, FFTTriggerUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _buf: IIdxSeq[AnyUGenIn] = buf.expand
      val _hop: IIdxSeq[AnyUGenIn] = hop.expand
...
case class PV_MagAbove(chain: AnyGE, thresh: AnyGE = 0.0f) extends SingleOutUGenSource[control, PV_MagAboveUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_MagBelow(chain: AnyGE, thresh: AnyGE = 0.0f) extends SingleOutUGenSource[control, PV_MagBelowUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
      val _thresh: IIdxSeq[AnyUGenIn] = thresh.expand
      val _sz_chain = _chain.size
...
case class PV_MagClip(chain: AnyGE, thresh: AnyGE = 0.0f) extends SingleOutUGenSource[control, PV_MagClipUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
      val _thresh: IIdxSeq[AnyUGenIn] = thresh.expand
      val _sz_chain = _chain.size
...
case class PV_MagMul(chainA: AnyGE, chainB: AnyGE) extends SingleOutUGenSource[control, PV_MagMulUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
...
case class PV_MagDiv(chainA: AnyGE, chainB: AnyGE, zeroes: AnyGE = 1.0E-4f) extends SingleOutUGenSource[control, PV_MagDivUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
...
case class PV_MagSquared(chain: AnyGE) extends SingleOutUGenSource[control, PV_MagSquaredUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_MagNoise(chain: AnyGE) extends SingleOutUGenSource[control, PV_MagNoiseUGen] with ControlRated with WritesFFT with UsesRandSeed {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_Copy(chainA: AnyGE, chainB: AnyGE) extends SingleOutUGenSource[control, PV_CopyUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _sz_chainA = _chainA.size
...
case class PV_CopyPhase(chainA: AnyGE, chainB: AnyGE) extends SingleOutUGenSource[control, PV_CopyPhaseUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _sz_chainA = _chainA.size
...
case class PV_PhaseShift(chain: AnyGE, shift: AnyGE) extends SingleOutUGenSource[control, PV_PhaseShiftUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_PhaseShift90(chain: AnyGE) extends SingleOutUGenSource[control, PV_PhaseShift90UGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_PhaseShift270(chain: AnyGE) extends SingleOutUGenSource[control, PV_PhaseShift270UGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_Min(chainA: AnyGE, chainB: AnyGE) extends SingleOutUGenSource[control, PV_MinUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _sz_chainA = _chainA.size
...
case class PV_Max(chainA: AnyGE, chainB: AnyGE) extends SingleOutUGenSource[control, PV_MaxUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _sz_chainA = _chainA.size
...
case class PV_Mul(chainA: AnyGE, chainB: AnyGE) extends SingleOutUGenSource[control, PV_MulUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _sz_chainA = _chainA.size
...
case class PV_Div(chainA: AnyGE, chainB: AnyGE) extends SingleOutUGenSource[control, PV_DivUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _sz_chainA = _chainA.size
...
case class PV_Add(chainA: AnyGE, chainB: AnyGE) extends SingleOutUGenSource[control, PV_AddUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _sz_chainA = _chainA.size
...
case class PV_RectComb(chain: AnyGE, numTeeth: AnyGE = 1.0f, phase: AnyGE = 0.0f, width: AnyGE = 0.5f) extends SingleOutUGenSource[control, PV_RectCombUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_RectComb2(chainA: AnyGE, chainB: AnyGE, numTeeth: AnyGE = 1.0f, phase: AnyGE = 0.0f, width: AnyGE = 0.5f) extends SingleOutUGenSource[control, PV_RectComb2UGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _numTeeth: IIdxSeq[AnyUGenIn] = numTeeth.expand
...
case class PV_BrickWall(chain: AnyGE, wipe: AnyGE) extends SingleOutUGenSource[control, PV_BrickWallUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_BinWipe(chainA: AnyGE, chainB: AnyGE, wipe: AnyGE) extends SingleOutUGenSource[control, PV_BinWipeUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _wipe: IIdxSeq[AnyUGenIn] = wipe.expand
...
case class PV_LocalMax(chain: AnyGE, thresh: AnyGE = 0.0f) extends SingleOutUGenSource[control, PV_LocalMaxUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
      val _thresh: IIdxSeq[AnyUGenIn] = thresh.expand
      val _sz_chain = _chain.size
...
case class PV_Conj(chain: AnyGE) extends SingleOutUGenSource[control, PV_ConjUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_BinScramble(chain: AnyGE, wipe: AnyGE = 0.5f, width: AnyGE = 0.2f, trig: AnyGE = 1.0f) extends SingleOutUGenSource[control, PV_BinScrambleUGen] with ControlRated with WritesFFT with UsesRandSeed {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
      val _wipe: IIdxSeq[AnyUGenIn] = wipe.expand
      val _width: IIdxSeq[AnyUGenIn] = width.expand
...
case class PV_MagSmear(chain: AnyGE, bins: AnyGE = 1.0f) extends SingleOutUGenSource[control, PV_MagSmearUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_MagShift(chain: AnyGE, stretch: AnyGE = 1.0f, shift: AnyGE = 0.0f) extends SingleOutUGenSource[control, PV_MagShiftUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PV_BinShift(chain: AnyGE, stretch: AnyGE = 1.0f, shift: AnyGE = 0.0f) extends SingleOutUGenSource[control, PV_BinShiftUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
      val _stretch: IIdxSeq[AnyUGenIn] = stretch.expand
      val _shift: IIdxSeq[AnyUGenIn] = shift.expand
...
case class PV_RandWipe(chainA: AnyGE, chainB: AnyGE, wipe: AnyGE, trig: AnyGE) extends SingleOutUGenSource[control, PV_RandWipeUGen] with ControlRated with WritesFFT with UsesRandSeed {
   protected def expandUGens = {
      val _chainA: IIdxSeq[AnyUGenIn] = chainA.expand
      val _chainB: IIdxSeq[AnyUGenIn] = chainB.expand
      val _wipe: IIdxSeq[AnyUGenIn] = wipe.expand
...
case class PV_Diffuser(chain: AnyGE, trig: AnyGE = 1.0f) extends SingleOutUGenSource[control, PV_DiffuserUGen] with ControlRated with WritesFFT with UsesRandSeed {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
      val _trig: IIdxSeq[AnyUGenIn] = trig.expand
      val _sz_chain = _chain.size
...
case class PV_RandComb(chain: AnyGE, wipe: AnyGE = 0.5f, trig: AnyGE = 1.0f) extends SingleOutUGenSource[control, PV_RandCombUGen] with ControlRated with WritesFFT with UsesRandSeed {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
      val _wipe: IIdxSeq[AnyUGenIn] = wipe.expand
      val _trig: IIdxSeq[AnyUGenIn] = trig.expand
...
case class PV_MagFreeze(chain: AnyGE, freeze: AnyGE = 1.0f) extends SingleOutUGenSource[control, PV_MagFreezeUGen] with ControlRated with WritesFFT {
   protected def expandUGens = {
      val _chain: IIdxSeq[AnyUGenIn] = chain.expand
...
case class PartConv[R <: Rate](rate: R, in: AnyGE, fftSize: AnyGE, buf: AnyGE) extends SingleOutUGenSource[R, PartConvUGen[R]] {
   protected def expandUGens = {
      val _in: IIdxSeq[AnyUGenIn] = in.expand
