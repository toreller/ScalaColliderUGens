...
private[synth] object FloatFun {
  ...
  @inline def round   (a: Float, b: Float): Float = if (b == 0) a else (math.floor(a / b + 0.5f) * b).toFloat
  @inline def roundup (a: Float, b: Float): Float = if (b == 0) a else (math.ceil(a / b) * b).toFloat
